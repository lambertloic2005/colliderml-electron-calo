...
class KinematicLoss(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "d0_mean",
            torch.tensor(stats["truth_d0"]["mean"], dtype=torch.float32),
        )
        self.register_buffer(
            "d0_std",
            torch.tensor(stats["truth_d0"]["std"], dtype=torch.float32),
        )
        ...
        # learned per-task log-sigma for the 5 REGRESSION tasks (eta, phi, logpt, z0, d0).
    ...
    def forward(self, pred, target, phi_centroid, eta_centroid, log_sum_et,
        ...
        pred_dlogpt  = pred[:, 2]
        pred_dz0     = pred[:, 3]
        pred_dd0     = pred[:, 4]      # z-scored d0 residual (anchor = d0 mean ~ 0)
        charge_logit = pred[:, 5]      # logit > 0 => predict positron (q = +1)
        ...
        target_dd0   = target[:, 4] * self.d0_std + self.d0_mean
        ...
        d0_target_norm = target_dd0 - self.d0_mean  # z-scored d0 residual
        ...
        d0_loss = F.huber_loss(pred_dd0, d0_target_norm, delta=1.0)
        ...
        # Homoscedastic weighting over the 5 REGRESSION tasks only.
        reg_losses = torch.stack([eta_loss, phi_loss, logpt_loss, z0_loss, d0_loss])
        ...
        dd0_err = (pred_dd0 - d0_target_norm) * self.d0_std            # back to mm           # back to mm
        ...
        logs = {
            ...
            "d0_rmse_mm": torch.sqrt(torch.mean(dd0_err ** 2)).detach(),
        }
