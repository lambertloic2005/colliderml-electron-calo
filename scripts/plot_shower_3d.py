"""CLI: 3D display of a single electron's calorimeter shower."""
import argparse
...
def main():
    ...
    p.add_argument("--electron-rank", type=int, default=0,
                   help="0 = highest-energy prompt electron, 1 = next, ...")
    p.add_argument("--direct-only", action="store_true",
                   help="Show only direct electron deposits (no descendants)")
    p.add_argument("--out", type=Path, default=Path("plots/shower_3d.png"))
    args = p.parse_args()
    ...
    print(f"Event {args.event_idx}: electron pid={e['particle_id']} "
          f"E={e['energy']:.2f} GeV (rank {args.electron_rank} of {len(electrons)})")
    fig = plots.shower_3d(p_row, c_row, e["particle_id"],
