...
D0_INDEX = TARGET_COLS.index("truth_d0")
...
def collect_predictions(model, loader, device):
    ...
    for batch in loader:
        ...
        target = batch["target"][:, [ETA_INDEX, PHI_INDEX, LOGPT_INDEX, Z0_INDEX, D0_INDEX]]
...
def main():
    ...
    d0_mean, d0_std = stats["truth_d0"]["mean"], stats["truth_d0"]["std"]
    ...
    pred_d0 = pred_norm[:, 4] * d0_std + d0_mean          # anchor = d0 mean (~0)       
    ...
    true_d0 = target_norm[:, 4] * d0_std + d0_mean
    ...
    d0_residual = pred_d0 - true_d0
    d0_fit = gaussian_resolution(d0_residual, wrap=False)
    plot_expected_vs_predicted(true_d0, pred_d0, "d0", output_dir, unit="mm")
    plot_residuals(d0_residual, "d0", output_dir, unit="mm", wrap=False)
    ...
    metrics = {
        ...
        "test/d0_rmse_mm": float(np.sqrt(np.mean(d0_residual**2))),
        "test/d0_mae_mm": float(np.mean(np.abs(d0_residual))),
        "test/d0_bias_mm": float(np.mean(d0_residual)),
        "test/d0_prior_rmse_mm": float(np.sqrt(np.mean((d0_mean - true_d0) ** 2))),
        "test/d0_sigma_mm": float(d0_fit.sigma),
    ...
    for label, lo, hi in REGIONS:
        ...
        d0_f = gaussian_resolution(d0_residual[m], wrap=False)
        metrics[f"test/region/{label}/d0_sigma_mm"]  = d0_f.sigma
        metrics[f"test/region/{label}/d0_rmse_mm"]   = float(np.sqrt(np.mean(d0_residual[m] ** 2)))
        metrics[f"test/region/{label}/d0_prior_mm"]  = float(np.std(true_d0[m]))
        plot_expected_vs_predicted(true_d0[m], pred_d0[m], f"d0_{label}", output_dir, unit="mm")
        plot_residuals(d0_residual[m], f"d0_{label}", output_dir, unit="mm", wrap=False)
